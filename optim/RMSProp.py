...
class RMSProp(Optim):
    ...
    def __init__(
        ...
        ]  # store the moving average of the squared gradient to scale the learning rate
        self.prev_velocities = [
            np.zeros_like(param.data) for param in self.params
        ]  # store the momentum coefficients for each parameter to apply momentum to the optimizer
        self.centered_accumulator = None
        if self.centered:
            self.centered_accumulator = [
                np.zeros_like(param.data) for param in self.params
            ]  # store the moving average of the gradient to compute the centered RMSProp, make the optimizer more stable and helps in taking bigger steps in steep regions
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            ...
            self.accumulator_value[i] = (
                self.alpha * self.accumulator_value[i] + (1 - self.alpha) * g**2
            )
            v_temp = self.accumulator_value[i]

            # Apply centered RMSProp
            if self.centered:
                self.centered_accumulator[i] = (
                    self.alpha * self.centered_accumulator[i] + (1 - self.alpha) * g
                )
                v_temp = v_temp - (self.centered_accumulator[i] ** 2)

            # Apply momentum
            if self.momentum != 0:
                self.prev_velocities[i] = self.momentum * self.prev_velocities[
                    i
                ] + g / (
                    np.sqrt(v_temp) + self.eps
                )  # update the velocity
