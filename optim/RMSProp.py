...
class RMSProp(Optim):
  ...
  def __init__(self, params: List[Tensor], lr=1e-2, alpha=0.99, eps=1e-8, weight_decay=0, momentum=0, centered=False):
    ...
    self.accumulator_value = [np.zeros_like(param.data) for param in self.params]
    self.prev_velocities = [np.zeros_like(param.data) for param in self.params]
    self.centerd_accumulator = [np.zeros_like(param.data) for param in self.params]
