...
class SGD(Optim):
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            ...
            # Apply momentum
            if self.momentum != 0:
                self.prev_velocities[i] = (
                    self.momentum * self.prev_velocities[i] + (1 - self.dampening) * g
                )

                # Apply Nesterov momentum
                if self.nesterov:
                    g += (
                        self.momentum * self.prev_velocities[i]
                    )  # Adds the future velocity to the gradient so that it provide the look ahead effect.
                else:
