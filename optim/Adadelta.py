...
class Adadelta(Optim):
  ...
  def __init__(self, params: List[Tensor], lr=1.0, rho=0.9, eps=1e-6, weight_decay=0):
    ...
    self.accumulator_value = [np.zeros_like(param.data) for param in self.params] # store the leaky average of the second moment of the gradient
    self.delta_accumulator = [np.zeros_like(param.data) for param in self.params] # store the leaky average of the second moment of the change in the parameters
