...
class KNeighborsRegressor(BaseEstimator):
    ...
    def __init__(self, n_neighbors=5, weights="uniform", p=2, metric="minkowski"):
        ...
        """
        super().__init__(
            "K Nearest Neighbors Regressor", "neighbors", mean_squared_error
        )
    ...
    def fit(self, X, y):
        ...
        assert len(X) == len(
            y
        ), "The number of input features and target values must be the same"

        self.data = (
            np.array(X, dtype=np.float64)
            if isinstance(X, np.ndarray)
            else X.to_numpy().astype(np.float64)
        )
        self.labels = (
            np.array(y, dtype=np.float64)
            if isinstance(y, np.ndarray)
            else y.to_numpy().astype(np.float64)
        )
    ...
    def predict(self, X):
        ...
        for x in X:
            ...
            if self.weights == "uniform":
                y_pred.append(
                    np.mean(idx_labels, axis=0)
                )  # compute the average of the target values of the k-nearest neighbours
            else:
                eps = 1e-12  # to avoid division by zero
                weights = 1 / (
                    distance[idx] + eps
                )  # compute the weights of each neighbour
                y_pred.append(
                    np.sum(weights * idx_labels, axis=0) / np.sum(weights, axis=0)
                )  # compute the weighted average of the target values of the k-nearest neighbours
