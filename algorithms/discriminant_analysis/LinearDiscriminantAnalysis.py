...
class LinearDiscriminantAnalysis(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if self.priors is not None:
            ...
            self.priors = np.bincount(y) / float(
                len(y)
            )  # Estimating the probability of each class from the data

        self.classes_ = np.unique(y)  # Unique classes in the target
        n_features, n_classes = X.shape[1], len(
            self.classes_
        )  # Number of features and classes
        self.means_ = np.zeros(
            (n_classes, n_features)
        )  # Mean of each feature per class
        self.covariance_ = np.zeros(
        ...
        for i, y_i in enumerate(self.classes_):
            ...
            self.covariance_ += (
                self.priors[i] * np.cov(X_i, bias=True, rowvar=False) * (n_i - 1)
            )  # Covariance matrix of the class, a weighted sum of the covariance of the class
    ...
    def predict(self, X):
        ...
        temp = np.dot(
            np.linalg.inv(self.covariance_), self.means_.T
        )  # (n_features, n_features) * (n_features, n_classes) = (n_features, n_classes)
        term1 = np.dot(
            X, temp
        )  # (n_samples, n_features) * (n_features, n_classes) = (n_samples, n_classes)
        term2 = -0.5 * np.sum(self.means_ * temp.T, axis=1) + np.log(
            self.priors
        )  # (n_classes,)
        decision = term1 + term2
        return self.classes_[
            np.argmax(decision, axis=1)
        ]  # Predicting the class with the highest decision value
