...
from utils import inertia, validate_convert_to_numpy

class KMeans(BaseUnsupervisedEstimator):
    ...
    def fit(self, X: Union[np.ndarray, pd.DataFrame]) -> None:
        ...
        self.centroids = self._random_init(X) if self.init == 'random' else self._kmeans_plus_plus_init(X)
    ...
    def clone(self) -> 'KMeans':
        """
        Clone the KMeans model
        
        Returns:
          - A clone of the KMeans model
        """
        return KMeans(self.n_clusters, init=self.init, max_iter=self.max_iter, tol=self.tol)
    ...
    def _assign_clusters(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(X.shape[0]):
            new_labels[i] = np.argmin(np.linalg.norm(X[i] - self.centroids, axis=1))
        return new_labels
