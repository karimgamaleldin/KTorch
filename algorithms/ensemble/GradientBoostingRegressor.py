...
class GradientBoostingRegressor(BaseEstimator):
  ...
  def fit(self, X, y):
    ...
    for _ in range(self.n_estimators):
      ...
      tree = DecisionTreeRegressor(max_depth=self.max_depth, min_samples_split=self.min_samples_split, max_features=self.max_features, min_impurity_decrease=self.min_impurity_decrease, ccp_alpha=self.ccp_alpha)
      tree.fit(X, r)
