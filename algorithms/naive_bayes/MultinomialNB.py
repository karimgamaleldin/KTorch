...
class MultinomialNB(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        for i, c in enumerate(self.classes):
            ...
            self.likelihood[i] = (np.sum(X_c, axis=0) + self.alpha) / (
                np.sum(X_c) + self.alpha * self.features
            )  # Calculate the likelihood probabilities of each feature given class c
    ...
    def predict(self, X):
        ...
        for i, _ in enumerate(self.classes):
            log_probs[i] = np.log(self.class_prior[i]) + np.sum(
                np.log(self.likelihood[i]) * X.T, axis=0
            )  # log_probs = log(prior) + sum(log(likelihood of each feature given class c)) * feature counts in X
