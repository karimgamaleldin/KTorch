...
class GaussianNB(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        self.class_var = np.array(
            [X[y == c].var(axis=0) + self.var_smoothing for c in self.classes]
        )  # Calculate variance of each class, add smoothing factor for numerical stability and better generalization
        return self

    def predict(self, X):
        n_samples = X.shape[0]
        posterior_log_likelihood = np.zeros(
            (n_samples, self.n_classes)
        )  # initialize posterior log likelihood (propotional to the posterior probability of each class given the data)
        for i in range(self.n_classes):
            ...
            posterior_log_likelihood[:, i] = (log_prob_term1 + log_prob_term2).sum(
                axis=1
            ) + self.log_prior[
                i
            ]  # axis=1 sums the log probabilities of the assumed independent features
