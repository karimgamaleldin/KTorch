...
class RidgeRegression(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        X_with_bias = (
            np.column_stack((X, np.ones_like(y))) if self.fit_intercept else X
        )  # Add a column of 1s to the input features if self.fit_intercept is True
        self.weights = (
            np.linalg.pinv(
                X_with_bias.T @ X_with_bias + self.alpha * np.eye(X_with_bias.shape[1])
            )
            @ X_with_bias.T
            @ y
        )  # Solve the normal equation with L2 regularization to get the Weights
        self.bias = (
            self.weights[-1] if self.fit_intercept else 0
        )  # Get the bias if self.fit_intercept is True
        self.weights = (
            self.weights[:-1] if self.fit_intercept else self.weights
        )  # Remove the bias if self.fit_intercept is True
