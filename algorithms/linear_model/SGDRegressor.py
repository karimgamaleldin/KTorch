...
class SGDRegressor(BaseEstimator):
    ...
    def __init__(
        ...
        assert self.loss in [
            ...
            "epsilon_insensitive",
            "squared_epsilon_insensitive",
        ], "loss must be one of 'squared_loss', 'huber', 'epsilon_insensitive', 'squared_epsilon_insensitive'"
    ...
    def fit(self, X, y):
        ...
        for i in range(self.max_iter):
            for j in range(n_samples):
                ...
                self.coef_ -= self.lr * (
                    gradient * X_j + self.regularization_gradient()
                )
    ...
    def gradient(self, y_true, y_pred):
        ...
        if self.loss == "squared_loss":
            ...
            error = y_true - y_pred
            grad += (
                self.epsilon * np.sign(error) if np.abs(error) <= self.epsilon else 0
            )
        elif self.loss == "squared_epsilon_insensitive":
            error = y_true - y_pred
    ...
    def regularization_gradient(self):
        ...
        if self.penalty == "l1":
            ...
            return (
                self.l1_ratio * np.sign(self.coef_) + (1 - self.l1_ratio) * self.coef_
            )
        return 0
    ...
    def regularization_term(self):
        ...
        if self.penalty == "l1":
            ...
            return self.l1_ratio * np.sum(np.abs(self.coef_)) + 0.5 * (
                1 - self.l1_ratio
            ) * np.sum(self.coef_**2)
        return 0
