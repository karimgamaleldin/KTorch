...
class LogisticRegression(BaseEstimator):
  ...
  def __init__(self, penalty='l2', tol=1e-4, alpha=1.0, max_iter=1000, l1_ratio=0.5, learning_rate=1e-4):
    '''
    ...
    assert self.l1_ratio is None or 0 <= self.l1_ratio <= 1, "l1_ratio must be between 0 and 1"
  ...
  def fit(self, X, y):
    ...
    self.w = np.random.randn(n_features, n_classes) * 0.01 # Initialize the weights with small random values
    self.b = np.zeros(n_classes)

    # Fit the model
    for i in range(self.max_iter):
      dot_product = X @ self.w + self.b # Calculate the dot product of X and the weights
      proba = softmax(dot_product) # Calculate the probabilities using the softmax function
      error = proba - np.eye(n_classes)[y.astype(int)] # Calculate the error (derivative of the loss function)
      gradient = X.T @ error
      ...
      if np.linalg.norm(gradient) < self.tol: # Check if the gradient is less than the tolerance
        break
  ...
  def regularization_gradient(self, gradient):
      if self.penalty == 'l2':
          ...
          return gradient + self.alpha * (self.l1_ratio * np.sign(self.w) + (1 - self.l1_ratio) * self.w)
      return gradient
