...
class TweedieRegressor(BaseEstimator):
  ...
  def __init__(self, *, power: float=0, alpha: float=0.0, fit_intercept: bool=True, link: str='auto', max_iter: int=1000, tol: float=1e-4):
    '''
    ...
    assert power in [0, 1, 2, 3, (1, 2)], 'power must be one of 0, 1, 2, 3, or (1, 2)'
    assert alpha >= 0, 'alpha must be non-negative'
    assert max_iter > 0, 'max_iter must be positive'
    assert tol > 0, 'tol must be positive'
    assert link in ['auto', 'identity', 'log'], 'link must be one of "auto", "identity" or "log"'
    self.power = power
    self.alpha = alpha
  ...
  def evaluate(self, X: np.ndarray, y: np.ndarray, metric=mean_squared_error) -> float:
  ...
  def clone(self):
    return TweedieRegressor(power=self.power, alpha=self.alpha, fit_intercept=self.fit_intercept, link=self.link, max_iter=self.max_iter, tol=self.tol)
