...
class SVC(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        K = self._kernel(
            X, X
        )  # (n_samples, n_samples) - contains the dot product of all samples in the z space
        ...
        support_vec_idx = (
            alphas > 1e-6
        )  # Support vectors are the ones with non-zero alphas, due to numerical precision 1e-6 is used
        ...
        temp = [
            self.labels[i] - np.sum(self.alphas * self.labels * K[support_vec_idx, i])
    ...
    def _solve_qp(self, K, y):
        ...
        q = matrix(
            -np.ones((n_samples, 1))
        )  # (n_samples, 1) - needed for the quadratic programming problem to convert the dual optimization maximization problem to a minimization problem, will be multiplied by the alphas
        G = matrix(
            np.vstack([-np.eye(n_samples), np.eye(n_samples)])
        )  # (2 * n_samples, n_samples) - inequality constraints, alphas >= 0 and alphas <= C, here we define wheather greater than the constraint or less than the constraint
        h = matrix(
            np.hstack([np.zeros(n_samples), np.ones(n_samples) * self.C])
        )  # (2 * n_samples, n_samples) - inequality constraints, alphas >= 0 and alphas <= C, here we define the numerical values of the constraints
        A = matrix(
            y.reshape(1, -1), (1, n_samples), "d"
        )  # (1, n_samples) - equality constraint, sum of alphas * y = 0 - this is the A
        b = matrix(
            np.zeros(1)
        )  # (1, 1) - equality constraint, sum of alphas * y = 0 - this is the 0
