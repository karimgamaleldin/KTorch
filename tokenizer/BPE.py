...
class BPE(Tokenizer):
    ...
    def train(self, corpus, vocab_size):
        ...
        for i in range(vocab_size):
            ...
            for i in range(len(corpus)):
                corpus[i] = self.merge_pair(
                    corpus[i], max_pair, token_id
                )  # Merge the most frequent pair of characters in each sentence
    ...
    def preprocess_corpus(self, corpus: List[str]) -> List[str]:
        ...
        for i in range(len(corpus)):
            ...
            new_sentence = []
            for word in sentence:
                encoded = self.convert_to_utf8(
                    word
                )  # Convert the word to UTF-8 encoding
                encoded = [
                    self.vocab.get(token, self.vocab[self.unk_token])
                    for token in encoded
                ]  # Convert the bytes to a list of ids
