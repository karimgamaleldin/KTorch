...
class LSTM(RNNBase):
  ...
  def __init__(self, input_size: int, hidden_size: int, num_layers: int, bias: bool = True, dropout: float = 0.0, bidirectional: bool = False):
    ...
    Initialize the LSTM
    '''
    super().__init__('LSTM', input_size, hidden_size, num_layers, bias, dropout, bidirectional)
    self.layers = [ LSTMCell(input_size if i == 0 else hidden_size, hidden_size, bias) for i in range(num_layers) ]
    self.dropout_layers = [ Dropout(dropout) for _ in range(num_layers) ]
    self.bi_layers = [ LSTMCell(input_size if i == 0 else hidden_size, hidden_size, bias) for i in range(num_layers) ] if bidirectional else None
