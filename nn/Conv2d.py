...
class Conv2d(nn.Module):
    ...
    def __init__(
        ...
        self.weight = simpleUniformInitialization(
            (out_channels, in_channels, kernel_size, kernel_size)
        )
        self.bias = (
            simpleUniformInitialization((out_channels, 1, 1, 1)) if bias else None
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        ), "Input tensor has incorrect number of channels"
        # Pad the input tensor
        x = KTorch.pad(x, self.padding, mode=self.padding_mode)

        # Apply the convolution operation
        out = x.conv2d(self.weight, self.stride)
        # Add the bias term
