...
class GRUCell(Module):
    ...
    def forward(self, x: Tensor, h: Tensor) -> Tensor:
        ...
        new_memory_content = KTorch.matmul(x, self.w_in) + (
            self.b_in if self.bias else 0
        )
        new_memory_content += reset_gate * (
            KTorch.matmul(h, self.w_hn) + (self.b_hn if self.bias else 0)
        )
    ...
    def parameters(self):
        ...
        if self.bias:
            out.extend(
                [self.b_ir, self.b_iz, self.b_in, self.b_hr, self.b_hz, self.b_hn]
            )
        return out
