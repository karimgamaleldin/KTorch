...
class BatchNorm1D(Module):
  ...
  def forward(self, x: Tensor) -> Tensor:
    ...
    if self.training:
      ...
      var = KTorch.var(x, axis=ax, keepdims=True, unbiased=False) # Biased variance like PyTorch
      if self.track_running_stats: # Check if running statistics should be tracked
        self.running_mean = (1 - self.momentum) * self.running_mean + self.momentum * mean
        temp_var = KTorch.var(x, axis=ax, keepdims=True, unbiased=True) # Unbiased variance like PyTorch
