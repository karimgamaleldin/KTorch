...
class BatchNorm2D(nn.Module):
  '''
  A class that represents the batch normalization layer, for 4D data 
  '''
  def __init__(self, num_features: int, eps: float = 1e-5, momentum: float = 0.1, affine: bool = True, track_running_stats: bool = True):
    ...
    self.running_mean = KTorch.zeros((1, num_features, 1, 1))
    self.running_var = KTorch.ones((1, num_features, 1, 1))
    self.gamma = KTorch.ones((1, num_features, 1, 1)) if affine else None
    self.beta = KTorch.zeros((1, num_features, 1, 1)) if affine else None
  ...
  def forward(self, x: Tensor) -> Tensor:
    ...
    if self.training:
      mean = KTorch.mean(x, axis=(0, 2, 3), keepdims=True)
      var = KTorch.var(x, axis=(0, 2, 3), keepdims=True, unbiased=False) # Biased variance like PyTorch
      if self.track_running_stats:
        self.running_mean = (1 - self.momentum) * self.running_mean + self.momentum * mean
        temp_var = KTorch.var(x, axis=(0, 2, 3), keepdims=True, unbiased=True)
