...
class RNNCell(Module):
    ...
    def __init__(
        ...
        self.w_ih = simpleUniformInitialization(
            (input_size, hidden_size)
        )  # to initialize correctly we need the order to be (output_size, input_size)
        self.w_hh = simpleUniformInitialization((hidden_size, hidden_size))
        self.b_ih = (
            simpleUniformInitialization((hidden_size, 1)).squeeze(-1) if bias else None
        )  # Instead of transposing the bias matrix, we can just add the bias in the forward pass
        self.b_hh = (
            simpleUniformInitialization((hidden_size, 1)).squeeze(-1) if bias else None
        )
        assert nonlinearity in [
            "tanh",
            "relu",
        ], "Nonlinearity must be either tanh or relu"
