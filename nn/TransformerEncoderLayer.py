...
class TransformerEncoderLayer(nn.Module):
  ...
  def forward(self, tgt, attn_mask=None, padding_mask=None):
    ...
    inp = tgt + inp
    inp = self.norm1(inp)
    inp2 = self.cross_attn(inp, tgt, tgt, attn_mask=attn_mask, padding_mask=padding_mask)
  ...
  def parameters(self):
    return self.mha.parameters() + self.cross_attn.parameters() + self.ffn.parameters() + self.norm1.parameters() + self.norm2.parameters() + self.norm3.parameters()