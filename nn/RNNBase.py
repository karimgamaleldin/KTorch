...
class RNNBase(Module):
  ...
  def __init__(self, mode: str, input_size: int, hidden_size: int, num_layers: int, bias: bool = True, dropout: float = 0.0, bidirectional: bool = False):
    '''
    Initialize the base RNN class
    '''
    super().__init__()
    self.mode = mode
    assert mode in ['RNN', 'LSTM', 'GRU'], 'The mode must be one of the following: RNN, LSTM, GRU'
  ...
  def forward(self, x: Tensor, h_0: Tensor=None, h_0_back: Tensor=None) -> Tensor:
    ...
    '''
    assert self.layers is not None or self.bi_layers is not None or self.dropout_layers is not None, 'Please use a subclass of RNNBase, such as RNN, LSTM, or GRU'
  ...
  def _forward_bidirectional(self, x: Tensor, h_0: Tensor, h_0_back: Tensor) -> Tensor:
    ...
    backward_out, backward_hidden = self._forward_single_direction(x.flip(1), h_0_back, isBi=True)
  ...
  def _forward_single_direction(self, x: Tensor, h_0: Tensor, isBi: bool = False) -> Tensor:
    '''
    ...
    if h_0 is None:
      if self.mode == 'LSTM':
        h_0 = [ (KTorch.zeros((batch_size, self.hidden_size)), KTorch.zeros((batch_size, self.hidden_size))) for _ in range(self.num_layers) ]
      else:
    ...
    for i in range(seq_len): # iterate over the sequence length
      ...
      for j in range(self.num_layers): # iterate over the number of layers
        if self.mode == 'LSTM':
          hidden, c = self.layers[j](x_i, h_prev[j]) if not isBi else self.bi_layers[j](x_i, h_prev[j])
        else:
          hidden = self.layers[j](x_i, h_prev[j]) if not isBi else self.bi_layers[j](x_i, h_prev[j])
        hidden = self.dropout_layers[j](hidden) 
        h_prev[j] = (hidden, c) if self.mode == 'LSTM' else hidden
        x_i = hidden

      output.append(x_i) # append the output of the current time step to the output list
