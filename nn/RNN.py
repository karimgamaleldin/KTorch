...
class RNN(RNNBase):
  ...
  def __init__(self, input_size: int, hidden_size: int, num_layers: int, nonlinearity: str = 'tanh', bias: bool = True, dropout: float = 0.0, bidirectional: bool = False):
    ...
    '''
    super().__init__('RNN', input_size, hidden_size, num_layers, bias, dropout, bidirectional)
    self.nonlinearity = nonlinearity
    self.layers = [RNNCell(input_size if i == 0 else hidden_size, hidden_size, bias, nonlinearity) for i in range(num_layers)]
    self.dropout_layers = [ Dropout(dropout) for _ in range(num_layers) ]
    self.bi_layers = [RNNCell(input_size if i == 0 else hidden_size, hidden_size, bias, nonlinearity) for i in range(num_layers)] if bidirectional else None
