...
class DecisionTreeInterface(BaseEstimator):
    ...
    def __init__(self, criterion, splitter, max_depth, min_samples_split, max_features, min_impurity_decrease, ccp_alpha, type, metric):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, sample_weight: np.ndarray = None):  # Added type annotations
        if self.max_features is None:
            self.max_features = X.shape[1]
        else:
            assert 0 < self.max_features <= X.shape[1], 'max_features should be in the range (0, n_features]'  # Added error message
        # Build the tree
    ...
    def evaluate(self, X: np.ndarray, y: np.ndarray, metric) -> float:  # Added type annotations
        predictions = self.predict(X)
        return metric(y_true=y, y_pred=predictions)

    def _build(self, X: np.ndarray, y: np.ndarray, depth: int = 0, sample_weight: np.ndarray = None) -> '_Node':  # Added type annotations
        ...
        feature, threshold, impurity = self._split(X, y, features, sample_weight=sample_weight)
    ...
    def _split(self, X: np.ndarray, y: np.ndarray, features: np.ndarray, sample_weight: np.ndarray = None):  # Added type annotations
        '''
    ...
    def _best_splitter(self, X: np.ndarray, y: np.ndarray, features: np.ndarray, sample_weight: np.ndarray = None):  # Added type annotations
        ...
        for feature in features:
            ...
            for threshold in thresholds:
                ...
                data_right_mask = ~data_left_mask
                # Calculate impurity
                left_impurity = self.criterion(y[data_left_mask], sample_weight=sample_weight) if self.type == 'classification' else self.criterion(y[data_left_mask], np.mean(y[data_left_mask]) if len(y[data_left_mask]) > 0 else 0)
                right_impurity = self.criterion(y[data_right_mask], sample_weight=sample_weight) if self.type == 'classification' else self.criterion(y[data_right_mask], np.mean(y[data_right_mask]) if len(y[data_right_mask]) > 0 else 0)
    ...
    def _random_splitter(self, X: np.ndarray, y: np.ndarray, features: np.ndarray):  # Added type annotations
        feature = np.random.choice(features)
        threshold = np.random.choice(np.unique(X[:, feature]))
        data_left_mask = X[:, feature] <= threshold
        data_right_mask = ~data_left_mask
        left_impurity = self.criterion(y[data_left_mask]) if self.type == 'classification' else self.criterion(y[data_left_mask], np.mean(y[data_left_mask]) if len(y[data_left_mask]) > 0 else 0)
        right_impurity = self.criterion(y[data_right_mask]) if self.type == 'classification' else self.criterion(y[data_right_mask], np.mean(y[data_right_mask]) if len(y[data_right_mask]) > 0 else 0)
...
class _Node:
    def __init__(self, feature: int = None, threshold: float = None, left: '_Node' = None, right: '_Node' = None, value: float = None):  # Added type annotations
